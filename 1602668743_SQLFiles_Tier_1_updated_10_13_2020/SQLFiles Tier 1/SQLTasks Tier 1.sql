...
SELECT DISTINCT CONCAT(m.firstname, ' ', m.surname, ' ', f.name) AS member_bookings
FROM `Members` AS m
...
WHERE starttime LIKE '2012-09-14%'
	AND f.membercost * slots > 30 AND b.memid != 0
UNION
SELECT 
	f.name AS facility, 
	CONCAT(m.firstname, ' ', m.surname) AS member_name,
	f.guestcost * slots AS cost
FROM `Bookings` AS b
LEFT JOIN `Facilities` AS f
ON b.facid = f.facid
LEFT JOIN `Members` AS m
ON b.memid = m.memid
WHERE starttime LIKE '2012-09-14%'
	AND f.guestcost * slots > 30 AND b.memid = 0
...
SELECT f.name, m.firstname || ' ' || m.surname AS member, SUM(b.slots) AS bookings
...
SELECT f.name, SUM(b.slots) AS bookings, Strftime('%m', b.starttime) AS month
        FROM `Facilities` AS f
        INNER JOIN `Bookings` AS b
        ON f.facid=b.facid
        WHERE b.memid <> 0
